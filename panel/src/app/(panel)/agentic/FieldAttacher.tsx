...
export interface AttachedField { collection: string; path: string; type: string }

interface SchemaField { name: string; types: string[] }
                      ...
                      {c.fields.map(f => {
                        ...
                        return (
                          <button key={f.name} type="button"
                            className={`flex items-center justify-between gap-2 px-2 py-1 rounded text-left text-xs hover:bg-panel2 ${on ? 'bg-accent/15 border border-accent-lo/40' : 'border border-transparent'}`}
                            onClick={() => toggleField(c.name, f)}>
                            <span className="font-mono text-ink-2 truncate">{f.name}</span>
